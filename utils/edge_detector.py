...
def process_segment_worker(
    ...
    # Directly use 'INITIAL_ERROR_THRESHOLD' as it's passed in config_params
    initial_error_threshold = config_params['INITIAL_ERROR_THRESHOLD']
    error_decay_rate = config_params.get('ERROR_DECAY_RATE', 0.0) # Keep fallback for decay rate if not set
    ...
    print(f"\n--- Segment {segment_id} (Depth {current_depth}) ---")
    print(f"  Segment Error Measure ({config_params['ERROR_MEASURE_TYPE']}): {segment_error_measure:.6f}")
    print(f"  Initial Error Threshold (from config): {initial_error_threshold}")
    print(f"  Error Decay Rate (from config): {error_decay_rate}")
    print(f"  Calculated Dynamic Threshold: {dynamic_error_threshold:.6f}")
    print(f"  Max Depth: {config_params['MAX_DEPTH']}")
    print(f"  Min Segment Size: {config_params['MIN_SEGMENT_SIZE']}")
    print(f"  Current Segment Size: {width}x{height}")
    ...
    if segment_error_measure <= dynamic_error_threshold: # Use the dynamic threshold here
        print(f"Worker Stopping for segment {segment_id}: Error measure {segment_error_measure:.6f} <= dynamic threshold {dynamic_error_threshold:.6f}. (TERMINATED BY ERROR)")
        results_data = {
            'bbox': segment_bbox,
            'coefficients': polynomial_coefficients_for_reconstruction,
            'depth': current_depth,
        ...
        return segment_id, results_data, None
         ...
         print(f"Worker Max depth ({config_params['MAX_DEPTH']}) reached for segment {segment_id}. Stopping recursion. (TERMINATED BY DEPTH)")
         results_data = {
             'bbox': segment_bbox,
             'coefficients': polynomial_coefficients_for_reconstruction,
             'depth': current_depth,
         ...
         print(f"Worker Segment size ({width}x{height}) below minimum ({config_params['MIN_SEGMENT_SIZE']}) for segment {segment_id}. Stopping recursion. (TERMINATED BY SIZE)")
         results_data = {
             'bbox': segment_bbox,
             'coefficients': polynomial_coefficients_for_reconstruction,
             'depth': current_depth,
        ...
        print(f"Worker Subdividing segment {segment_id}: Error measure {segment_error_measure:.6f} > dynamic threshold {dynamic_error_threshold:.6f}.")
...
class AdaptivePolynomialReconstructor:
    ...
    def reassemble_raw_error_maps_for_all_types(self, upscale_factor: int = 1) -> None:
        ...
        Crucially, this version sets error values to zero for segments that
        terminated due to low error (i.e., were smooth regions).
        """
        if self.original_image_shape is None:
        ...
        for segment_id, results in self.final_segment_data.items():
            if 'bbox' in results and 'raw_error_maps' in results and 'status' in results:
                row_start, row_end, col_start, col_end = results['bbox']
                segment_raw_error_maps = results['raw_error_maps']
                segment_status = results['status'] # Get the termination status
                ...
                for err_type in error_map_types:
                    if err_type in segment_raw_error_maps and segment_raw_error_maps[err_type].size > 0:
                        ...
                        # --- NEW LOGIC: Zero out errors for segments terminated by low error ---
                        if segment_status == 'terminated_by_error':
                            resized_segment_error.fill(0.0) # Set all error values in this segment to zero
        ...
        print("Reassembled all raw error maps (filtered for smooth regions).")
    ...
    def run_reconstruction(self):
        ...
        worker_config_params = {
            ...
            # Pass the new dynamic threshold parameters
            'INITIAL_ERROR_THRESHOLD': self.config.INITIAL_ERROR_THRESHOLD,
        ...
        # --- DEBUG PRINT ---
        print(f"Main process: worker_config_params being sent: {worker_config_params}")
        # --- END DEBUG PRINT ---
        ...
        if self.config.SAVE_SEGMENT_DATA and self.final_segment_data:
            ...
            params_suffix = (
                ...
                f"_initErr{str(self.config.INITIAL_ERROR_THRESHOLD).replace('.', 'p')}"
                f"_decay{str(self.config.ERROR_DECAY_RATE).replace('.', 'p')}"
        ...
        if self.final_segment_data:
            ...
            if reassembled_approx_image.size > 0:
                ...
                # This call now includes the logic to zero out errors for smooth segments
                ...
                if self.reassembled_raw_error_maps:
                    print("\nCombining reassembled error maps for edge detection (excluding logical operations)...")
                    
                    # Normalize the filtered reassembled errors for combination
                    normalized_filtered_reassembled_errors = {
                    ...
                    if normalized_filtered_reassembled_errors:
                        # Only use 'max' or 'weighted_sum' strategies
                        self.reassembled_composite_error_map = combine_errors(
                            normalized_filtered_reassembled_errors,
                            strategy=self.config.ERROR_COMBINATION_STRATEGY, # This should now only be 'max' or 'weighted_sum'
                        ...
                        print("No normalized and filtered reassembled error maps to combine. Composite map will be empty.")
                ...
                params_suffix = ( # Regenerate suffix to ensure consistency
                    ...
                    f"_edgeComb-{self.config.ERROR_COMBINATION_STRATEGY}" # Simplified suffix for edge combination
                    f"_edgeThreshType-{self.config.EDGE_THRESHOLD_TYPE}" # Simplified suffix for edge threshold type
                    + (f"-{str(self.config.FIXED_EDGE_THRESHOLD).replace('.', 'p')}" if self.config.EDGE_THRESHOLD_TYPE == 'fixed' else '')
                    + "_filtered" # Indicate that smooth regions are filtered
                )
                ...
                if self.config.SAVE_RAW_ERROR_MAPS and self.reassembled_raw_error_maps: # Use self.config
                    print("\nSaving reassembled raw error maps (filtered for smooth regions)...")
                    for key, err_map in self.reassembled_raw_error_maps.items():
                        if err_map is not None and err_map.size > 0:
                            ...
                            fig_raw_err.colorbar(im_raw_err, ax=ax_raw_err, label=f'Normalized Filtered Raw Error ({key.replace("_", " ").title()})')
                            ax_raw_err.set_title(f'Reassembled Filtered Raw Error Map: {key.replace("_", " ").title()}')
                            ax_raw_err.axis('off')
                            try:
                                plt.savefig(plot_filepath_raw_err)
                                print(f"Saved reassembled filtered raw error map '{key}' plot to {plot_filepath_raw_err}")
                            except Exception as e:
                                print(f"Error saving reassembled filtered raw error map '{key}' plot: {e}")
                ...
                if self.config.SAVE_COMPOSITE_ERROR_MAP and self.reassembled_composite_error_map is not None and self.reassembled_composite_error_map.size > 0: # Use self.config
                    print("\nSaving reassembled composite error map (filtered)...")
                    ...
                    fig_composite_err.colorbar(im_composite_err, ax=ax_composite_err, label=f'Normalized Composite Error ({self.config.ERROR_COMBINATION_STRATEGY.capitalize()})')
                    ax_composite_err.set_title(f'Reassembled Composite Error Heatmap ({self.config.ERROR_COMBINATION_STRATEGY.capitalize()}, Filtered)')
                ...
                if self.config.SAVE_BINARY_EDGE_MAPS and self.final_adaptive_binary_edge_map is not None and self.final_adaptive_binary_edge_map.size > 0: # Use self.config
                    print("\nSaving final adaptive binary edge map (filtered)...")
                ...
                if self.config.SAVE_MAIN_PLOT: # Assuming SAVE_MAIN_PLOT also controls this, or you can add a new config flag
                    ...
                    if self.reassembled_composite_error_map is not None and self.reassembled_composite_error_map.size > 0:
                        ...
                        ax_chosen_error.set_title(f'Chosen Error Map ({self.config.ERROR_COMBINATION_STRATEGY.capitalize()}, Filtered)')
                ...
                if self.config.SAVE_EDGE_STRATEGY_COMPARISON_PLOT and self.config.EDGE_STRATEGIES_TO_COMPARE:
                    ...
                    if num_strategies == 0:
                        ...
                        # Normalize the filtered reassembled errors for comparison
                        normalized_filtered_reassembled_errors = {
                            key: normalize_error_image(err_map)
                            for key, err_map in self.reassembled_raw_error_maps.items()
                            if err_map is not None and err_map.size > 0
                        ...
                        for i, strategy_name in enumerate(self.config.EDGE_STRATEGIES_TO_COMPARE):
                            ...
                            # Only use 'max' or 'weighted_sum' for comparison
                            temp_composite_error_map = combine_errors(
                                normalized_filtered_reassembled_errors,
                                strategy=strategy_name, # This should now only be 'max' or 'weighted_sum'
                                weights=self.config.ERROR_COMBINATION_WEIGHTS
                            )
                        ...
                        for i, strategy_name in enumerate(self.config.EDGE_STRATEGIES_TO_COMPARE):
                            ...
                            ax.set_title(title_text + "\n(Filtered)") # Indicate filtering in title
                            ax.axis('off')
